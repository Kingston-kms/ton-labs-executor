...
fn test_computing_phase_acc_notexist_extmsg_nostate() {
    ...
    let (phase, _actions) = compute_phase(Some(&msg), &mut acc, &info, &executor, &GasConfigFull::default_mc(), false, true).unwrap();
    assert_eq!(
        phase, 
...
fn test_computing_phase_acc_notexist_intmsg_state() {
    ...
    let config = BlockchainConfig::default();
    let executor = OrdinaryTransactionExecutor::new(config);
    let (phase, _actions) = compute_phase(Some(&msg), &mut acc, &info, &executor, &gas_config, false, true).unwrap();
...
fn test_computing_phase_acc_uninit_intmsg_with_nostate() {
    ...
    let config = BlockchainConfig::default();
    let executor = OrdinaryTransactionExecutor::new(config);
    let (phase, _actions) = compute_phase(Some(&msg), &mut acc2, &info, &executor, &gas_config, false, true).unwrap();
    assert_eq!(
        phase, 
        TrComputePhase::Skipped(TrComputePhaseSkipped { reason: ComputeSkipReason::NoState })
    );
...
fn test_computing_phase_activeacc_gas_not_accepted() {
    ...
    let config = BlockchainConfig::default();
    let executor = OrdinaryTransactionExecutor::new(config);
    let result = compute_phase(Some(&msg), &mut acc, &info, &executor, &gas_config, false, true);
    println!("{:?}", result);
    if let Err(e) = result {
...
fn call_action_phase(
    ...
    let phase = action_phase(&mut tr, &mut acc, Some(actions.write_to_new_cell().unwrap().into()), &config, tr_lt, false).unwrap();
